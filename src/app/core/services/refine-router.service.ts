...
import type { ActionType, ResourceConfig } from '@core/types'
...
export class RefineRouterService {
  generateRoutes(resources: ResourceConfig[]): Route[] {
    ...
    for (const resource of resources) {
      ...
      if (resource.children) {
        routes.push(
          ...this.createNestedResourceRoutes(resource, resource.children)
        )
      }
    }
  ...
  private createResourceRoutes(resource: ResourceConfig): Route[] {
    ...
    if (resource.list) {
      ...
      const hasModalActions =
        resource.meta?.createInModal || resource.meta?.editInModal
  ...
  private createNestedResourceRoutes(
